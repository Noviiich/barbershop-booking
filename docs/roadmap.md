...
Дата: 2026-09-11. PR-00/01 выполнены, PR-02 реализован с незавершённой Docker-проверкой MIG-01, PR-03 реализован с пройденным AUTH-01, PR-04 реализован с ожидающими Docker-проверками MIG-01/CAT-01. PR-05–PR-26 — будущие работы; их выполнение требует отдельного запроса на реализацию. Номера обозначают логические шаги, а не уже открытые pull requests. Каждый шаг имеет одну основную цель; при расширении scope его нужно разделить, сохранив зависимости и критерии.
...
Acceptance criteria: пересечение разных начал и буферов одного глобального мастера запрещено даже между филиалами; соседние интервалы и разные мастера разрешены; CANCELLED не блокирует, COMPLETED/NO_SHOW сохраняют ограничение; пустые/null/несогласованные интервалы невозможны.
...
Изменения: read service каталога/слотов, сетка старта, фильтры мастера/услуги, границы диапазона; исключение занятости с буферами.
...
Автоматический тест: MOVE-01 — перенос/create, перенос/cancel, два переноса, rollback и буферы.
