...
Дата: 2026-09-16. Реализованы проверки PR-01–PR-16, включая API-01, OUT-01/02 и первую
часть YAN-01 для snapshot/fixtures/capability gates. DB-проверки выполняются на PostgreSQL 17.
OBS-01 реализован для alert/redaction локального канала; техническая часть SEC-01 покрывает
tombstone и privacy ledger. PR-18–PR-20 ожидают Yandex gates. Расширения существующих
семейств и остальные тестовые ID/пути ниже — спецификация будущих тестов.
...
| AUTH-01 | Мастер/администратор не получают чужой филиал; роли не повышаются через payload; replay заново проверяет scope | `tests/security/test_permissions.py` | 03, 14, 18; PR-03 реализован |
...
## Команды приложения и будущих этапов

Команды запускаются из корня репозитория. Будущие команды станут обязательствами
соответствующих PR. Каждый PR добавляет именно используемые им targets, fixtures и тесты;
`pytest` с нулём собранных тестов считается ошибкой, а не успешной проверкой. В CI RTK
необязателен: выполняется вложенная команда.
...
| `rtk proxy make contract-check` | PR-16, реализована | Проверка snapshot/fixtures без сети и production credentials |
